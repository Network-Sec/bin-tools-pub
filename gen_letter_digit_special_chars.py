...
# In the example is only one special char (exclamation) but below we use all common 
# special chars used in passwords. hashcat doesn't provide this unique combination
# and erroers when we try to generate such a list with --stdout. There's custom
# combos like -1 ?l?u?d but the custom charset isn't supported in all modes.
